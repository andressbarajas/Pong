...
namespace PongaThemes
{
    ...
    class PongGameplayScreen : GameScreen
    {
        ...
        // Needed
        Rectangle boxrec = new Rectangle(HelperUtils.SafeBoundary.X + 150, HelperUtils.SafeBoundary.Y, 724, 612);
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            // Draw Scores
            spriteBatch.DrawString(m_score_fnt, m_player_1_score.ToString(), new Vector2(HelperUtils.SafeBoundary.X + 332 - m_score_fnt.MeasureString(m_player_1_score.ToString()).X, HelperUtils.SafeBoundary.Y + 100), Color.White);
            spriteBatch.DrawString(m_score_fnt, m_player_2_score.ToString(), new Vector2(HelperUtils.SafeBoundary.X + 844 - m_score_fnt.MeasureString(m_player_2_score.ToString()).X, HelperUtils.SafeBoundary.Y + 100), Color.White);

            int y = HelperUtils.SafeBoundary.Y + 3;

            for (int i = 0; i < 31; i++)
            {
                spriteBatch.Draw(ScreenTexture, new Rectangle((ScreenManager.GraphicsDevice.Viewport.Width / 2) - 1, y, 3, 10), Color.White);
                y += 20;
            }

            #if XBOX // Draw the WideScreen Lines (Top and bottom)
            //Top
            spriteBatch.Draw(ScreenTexture, new Rectangle(0, boxrec.Top - 63, 1024 + 2 * HelperUtils.SafeBoundary.X, 63), Color.White);
            //Bottom
            spriteBatch.Draw(ScreenTexture, new Rectangle(0, boxrec.Bottom, 1024 + 2 * HelperUtils.SafeBoundary.X, 63), Color.White);
            #endif
